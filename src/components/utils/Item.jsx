        ...
        <div className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists ? ' justify-items-start' : ' justify-items-center'} rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
            <div className={`grid items-center ${ifExists ? ' justify-items-start' : ' justify-items-center'}`}>
                ...
                <div className='flex items-center gap-3'>
                    <button type='button' onClick={() => onAddToCart()} className='bg-white/90 blur-effect-theme button-theme p-0.5 shadow shadow-slate-200'><ShoppingBagIcon className='icon-style text-slate-900'/></button>
                    <button type='button' onClick={() => {onAddToCart(); onCartToggle()}} className='bg-white/90 blur-effect-theme button-theme px-2 py-1 shadow shadow-slate-200 text-sm text-black'>{btn}</button>
                </div>
            </div>
            <div className={`flex items-center ${ifExists ? 'absolute top-5 right-1' : ' justify-center'}`}>
                <img
                    src={img}
                    alt={`img/item-img/${id}`}
                    className={`transition-theme hover:-rotate-12 ${ifExists ? 'h-auto w-56 md:w-48 -rotate-[35deg]' : 'h-36 w-64'}`}
                />
            </div>
        </div>
    </>
